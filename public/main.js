...
function renderPortfolio() {
    ...
    if (marquee) {
        const content = PORTFOLIO_DATA.skills.map(s => `<div class="flex items-center"><span class="mx-6 text-white">✦</span><span>${s}</span></div>`).join('');
    ...
    if (expList) {
        expList.innerHTML = PORTFOLIO_DATA.experience.map(exp => `
    ...
    container.innerHTML = PORTFOLIO_DATA.projects.map((proj, index) => {
        const isEven = index % 2 === 0;
        return `
        <div class="project-row group flex items-center justify-between w-full ${isEven ? 'flex-row' : 'flex-row-reverse'}">
            <div class="timeline-number font-black">PROJ_${(index + 1).toString().padStart(2, '0')}</div>
            ...
            <div class="project-info-side w-[45%] ${isEven ? 'pl-12 text-left' : 'pr-12 text-right'}">
                <div class="flex flex-wrap gap-2 mb-4 ${isEven ? 'justify-start' : 'justify-end'}">
    ...
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(messages.whatsapp)}`;
    const mailUrl = `mailto:${email}?subject=${encodeURIComponent(messages.emailSubject)}&body=${encodeURIComponent(messages.emailBody)}`;

    const navEmail = document.getElementById('nav-email-link');
    if (navEmail) navEmail.href = mailUrl;

    const heroTalk = document.getElementById('hero-wa-link');
    if (heroTalk) heroTalk.href = waUrl;
    ...
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
        <div class="flex items-center gap-6">
            ...
            <a href="${mailUrl}" class="text-sm font-medium text-gray-400 hover:text-[#fb92c4] transition-colors italic">${email}</a>
        </div>
        ...
        <div class="flex items-center gap-6">
            ...
            <a href="${waUrl}" target="_blank" rel="noopener noreferrer" class="social-icon-link" title="WhatsApp">
    ...
    `;
    // Re-inisialisasi lucide icons setelah di-render
    lucide.createIcons();
}
...
function setupHeroActions() {
    const { phone, messages } = PORTFOLIO_DATA.contact;
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(messages.whatsapp)}`;

    // Set link WA ke tombol Let's Talk di Hero
    const heroWaLink = document.getElementById('hero-wa-link');
    if (heroWaLink) {
        heroWaLink.href = waUrl;
    }
}
